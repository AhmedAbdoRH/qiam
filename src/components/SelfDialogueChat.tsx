                ...
                >
                  <div
                    className={`inline-block p-2 rounded-2xl break-words ${msg.sender === 'me'
                      ...
                      : `${animaColors.msgBg} ${animaColors.msgText} rounded-br-sm border ${animaColors.msgBorder} ${animaColors.msgShadow}`
                      }`}
                  >
                    <p className="text-xs leading-tight whitespace-pre-wrap" style={{ unicodeBidi: 'plaintext' }}>{msg.message}</p>
                  </div>
                  <div className={`flex items-center gap-0.5 mt-0.5 ${msg.sender === 'me' ? 'justify-start' : 'justify-end'}`}>
                    {msg.sender === 'me' ? (
                      ...
                      <Heart className={`h-2 w-2 ${animaColors.iconColor}`} />
                    )}
                    <span className={`text-[7px] ${msg.sender === 'me' ? 'text-blue-400/15' : animaColors.timeColor}`}>
                      {msg.sender === 'me' ? 'أنا' : (isNurturing ? 'الراعية' : 'الأنيما')} • {formatTime(msg.created_at)}
                    </span>
